...
export function HomepagePhotoManager({ photos }: { photos: HomepagePhoto[] }) {
  ...
  useEffect(() => {
    if (uploadState.ok) uploadForm.current?.reset();
  }, [uploadState.ok]);

  function move(id: string, direction: "up" | "down") {
    startTransition(async () =>
      setManageState(await moveHomepagePhoto(id, direction)),
    );
  }
  ...
  return (
    <section
      ...
      style={{ marginTop: 30, padding: "clamp(22px, 6vw, 34px)" }}
    >
      <p className="eyebrow">homepage photographs</p>
      <h2
        className="serif"
        style={{ margin: "8px 0", fontSize: 32, fontWeight: 500 }}
      >
        the little gallery at the top
      </h2>
      <p className="muted" style={{ lineHeight: 1.6 }}>
        choose a few photographs that feel like home. the first one here appears
        first.
      </p>
      ...
      >
        ...
        <label className="field">
          ...
          <input
            ...
            name="altText"
            maxLength={240}
            placeholder="us by the water on a cloudy afternoon"
            required
          />
          <small className="muted">
            describe what is visible for someone who cannot see the photograph.
          </small>
        </label>
        <label className="field">
          <span>
          ...
          <input
            ...
            name="caption"
            maxLength={240}
            placeholder="one of my favourite days"
          />
        </label>
        <button className="button small" disabled={uploading}>
          {uploading ? "keeping this safe…" : "add photograph"}
        </button>
        ...
        {photos.map((photo, index) => (
          <article key={photo.id} className="photo-manage-row">
            ...
            <div style={{ minWidth: 0 }}>
              <strong>{photo.caption || photo.alt_text}</strong>
              <small
                className="muted"
            ...
            <div className="photo-manage-actions">
              <button
                ...
                aria-label={`move ${photo.caption || "photograph"} earlier`}
              >
                ↑
              </button>
              <button
                ...
                aria-label={`move ${photo.caption || "photograph"} later`}
              >
                ↓
              </button>
              <button
                ...
                disabled={pending}
                aria-label={`remove ${photo.caption || "photograph"}`}
              >
                remove
              </button>
            </div>
      ...
      <dialog ref={removeDialog} className="confirm-dialog">
        ...
        <p className="muted">
          the private file and its caption will be permanently removed from the
          homepage.
        </p>
        <div
        ...
        >
          ...
          <button
            ...
            type="button"
            onClick={() => removeDialog.current?.close()}
            disabled={pending}
          >
            keep it
          </button>
