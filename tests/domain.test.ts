...
test("Helsinki daylight saving changes its distance from Singapore", () => {
  ...
  assert.equal(
    zonedHour("Asia/Singapore", winter) - zonedHour("Europe/Helsinki", winter),
    6,
  );
  assert.equal(
    zonedHour("Asia/Singapore", summer) - zonedHour("Europe/Helsinki", summer),
    5,
  );
});
...
test("migration keeps authorization, read receipts, and storage server-side", async () => {
  const sql = await readFile(
    new URL("../supabase/migrations/202608190001_initial.sql", import.meta.url),
    "utf8",
  );
  assert.match(sql, /alter table public\.cards enable row level security/);
  assert.match(
    sql,
    /create policy "creators can delete cards"[\s\S]*using \(creator_id = auth\.uid\(\)\)/,
  );
  assert.match(
    sql,
    /recipient_id = auth\.uid\(\) and public\.card_is_unlocked\(cards\)/,
  );
  assert.match(
    sql,
    /c\.recipient_id = auth\.uid\(\) and public\.card_is_unlocked\(c\)/,
  );
...
test("love pings have no cooldown", async () => {
  const route = await readFile(
    new URL("../app/api/thinking-of-you/route.ts", import.meta.url),
    "utf8",
  );
  const migration = await readFile(
    new URL(
      "../supabase/migrations/202608210002_remove_love_ping_cooldown.sql",
      import.meta.url,
    ),
    "utf8",
  );
  ...
  assert.match(
    migration,
    /drop function if exists public\.reserve_love_ping\(uuid, integer\)/,
  );
...
test("local account preview is allowlisted and exchanges only a magic-link token", async () => {
  const script = await readFile(
    new URL("../scripts/login-as.ts", import.meta.url),
    "utf8",
  );
  ...
  assert.match(
    pkg,
    /"login-as": "node --env-file=\.env\.local --import tsx scripts\/login-as\.ts"/,
  );
});
...
test("Telegram webhook rejects invalid secrets and ignores unauthorized, bot, and non-text updates", async () => {
  ...
  const request = (body: unknown, secret = "expected-secret") =>
  ...
  try {
    assert.equal(
      (await POST(request({ update_id: 1 }, "wrong-secret"))).status,
      401,
    );
    assert.equal(
      (
...
test("Telegram group authorization rejects private, bot, and unallowlisted senders", () => {
  ...
  assert.equal(
    isAllowedTelegramParticipant(group, user, "-303", allowed),
    true,
  );
  assert.equal(
    isAllowedTelegramParticipant(
  ...
  assert.equal(
    isAllowedTelegramParticipant(group, { ...user, id: 999 }, "-303", allowed),
    false,
  );
  assert.equal(
    isAllowedTelegramParticipant(
...
test("Telegram reset, numeric limits, and secret redaction are bounded", () => {
  ...
  assert.equal(
    telegramResetRequested("/reset@our_love_bot", "our_love_bot"),
    true,
  );
  assert.equal(
    telegramResetRequested("/reset@another_bot", "our_love_bot"),
    false,
  );
  assert.equal(parseTelegramInteger("5000", 10, 1, 100), 100);
  assert.equal(parseTelegramInteger("invalid", 10, 1, 100), 10);
  assert.equal(
    redactTelegramSecrets("key-test and -303", ["key-test", "-303"]),
...
test("Telegram context has no client access policy", async () => {
  const sql = await readFile(
    new URL(
      "../supabase/migrations/202608200002_telegram_context.sql",
      import.meta.url,
    ),
    "utf8",
  );
...
test("Telegram update claims atomically enforce idempotency and limits", async () => {
  const sql = await readFile(
    new URL(
      "../supabase/migrations/202608210001_telegram_llm_controls.sql",
      import.meta.url,
    ),
    "utf8",
  );
  ...
  for (const name of [
    "OPENAI_API_KEY",
    "OPENAI_MODEL",
    "TELEGRAM_LLM_COOLDOWN_SECONDS",
    "TELEGRAM_LLM_DAILY_LIMIT",
  ]) {
...
test("Telegram webhook LLM is disabled and replies are not generated", async () => {
  ...
  assert.match(webhook, /LLM replies are disabled/);
  assert.doesNotMatch(
    webhook,
    /reserve_telegram_update|tiny pause|generateReply|new OpenAI/,
  );
});
...
test("homepage photographs stay private and creator-managed", async () => {
  const sql = await readFile(
    new URL(
      "../supabase/migrations/202608200001_homepage_photos.sql",
      import.meta.url,
    ),
    "utf8",
  );
...
test("zonedDate returns the calendar date in the given timezone", () => {
  ...
  assert.equal(
    zonedDate("Europe/Helsinki", new Date("2026-08-21T23:30:00Z")),
    "2026-08-22",
  );
  // 2026-08-21 20:00 UTC = 2026-08-21 23:00 Helsinki (still Aug 21)
  assert.equal(
    zonedDate("Europe/Helsinki", new Date("2026-08-21T20:00:00Z")),
    "2026-08-21",
  );
  // Same instant in Singapore (UTC+8) is already Aug 22
  assert.equal(
    zonedDate("Asia/Singapore", new Date("2026-08-21T16:30:00Z")),
    "2026-08-22",
  );
});
...
test("zonedDate handles winter (EET UTC+2) and summer (EEST UTC+3) correctly", () => {
  ...
  assert.equal(
    zonedDate("Europe/Helsinki", new Date("2026-01-15T22:30:00Z")),
    "2026-01-16",
  );
  assert.equal(
    zonedDate("Europe/Helsinki", new Date("2026-01-15T21:59:00Z")),
    "2026-01-15",
  );
  // Summer: midnight Helsinki = 21:00 UTC previous day
  assert.equal(
    zonedDate("Europe/Helsinki", new Date("2026-07-15T21:30:00Z")),
    "2026-07-16",
  );
  assert.equal(
    zonedDate("Europe/Helsinki", new Date("2026-07-15T20:59:00Z")),
    "2026-07-15",
  );
});
...
test("countdown cron fires once daily (no hourly midnight gate)", async () => {
  const vercelJson = await readFile(
    new URL("../vercel.json", import.meta.url),
    "utf8",
  );
  assert.match(vercelJson, /0 21 \* \* \*/);
  const route = await readFile(
    new URL("../app/api/cron/countdown/route.ts", import.meta.url),
    "utf8",
  );
...
test("countdown sends only for milestones and Finland-date occasions", async () => {
  ...
  assert.equal(
    countdownMessage("2026-09-21", 90),
    "good morning from singapore ♡ only 90 days until we're together again.\n3 months left.",
  );
  assert.equal(
    countdownMessage("2026-10-03", 78),
    "good morning from singapore ♡ only 78 days until we're together again.\nhappy 30 months to us ♡",
  );
  assert.equal(
  ...
  assert.equal(
    ...
    "good morning from singapore ♡ today is the day — we made it.",
  );
  assert.equal(countdownMessage("2026-12-21", -1), null);
});
...
test("countdown route uses CRON_SECRET, timezone checks, and idempotency table", async () => {
  ...
  assert.match(route, /no-milestone/);
...
test("countdown delivery table has RLS and no browser access", async () => {
  const sql = await readFile(
    new URL(
      "../supabase/migrations/202608210003_countdown_deliveries.sql",
      import.meta.url,
    ),
    "utf8",
  );
...
test("countdown cron route never logs or returns secrets", async () => {
  ...
  assert.doesNotMatch(
    route,
    /console\.\w+\([^)]*(?:token|CRON_SECRET|process\.env)/,
  );
