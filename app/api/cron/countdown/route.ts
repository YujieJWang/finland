...
import { countdownMessage } from "@/lib/countdown";
...
export async function GET(request: NextRequest) {
  ...
  const message = countdownMessage(todayInHelsinki, daysRemaining);
  if (!message) {
    return NextResponse.json({ ok: true, skipped: "no-milestone" });
  }
